...
**v2 학습 구조** (`PLAN.md`): `CONFIG["trainer"]` 로 고른다. 오라클·스모크 게이트 통과(2026-09-06) 후 기본값은 `"v2"` 이고 `"v1"` 은 예전 경로(gymnasium 필요)다. 보상은 `grpo/oracle.py` 의 FFT 플립 오라클(모든 픽셀의 ΔPSNR 을 한 번에, 근사 없이)이고, 정책은 복소 필드 특징(51채널) 위의 unet/fcn/fno 이며, K 장 × G 표본 배치 GRPO 로 학습한다. 산출물은 `grpo_models_v2_<policy_kind>/`, 체크포인트 `grpo_v2_it<N>.pt`. 서버에서 돌리기 전 순서: `python grpo/oracle_selftest.py`(오라클 vs `tt.simulate` 대조, 통과 필수) → `python grpo/smoke_v2.py`(작은 크기 전체 경로) → v2 짧은 런. 로컬(torch 없이)에서는 `python grpo/oracle_algebra_check_np.py` 로 오라클 대수를 검증한다. 평가 표에 오라클 탐욕 상한을 넣으려면 `eval_checkpoints.py` 의 `INCLUDE_ORACLE_GREEDY=True`; 체크포인트 선택과 보고를 다른 이미지로 나누려면 `EVAL_SPLIT`, 시드 반복은 `EVAL_SEEDS`.
